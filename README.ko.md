...
[최신 릴리즈 페이지](https://github.com/baserize/full-brightness/releases/latest)에서 notarize된 DMG를 내려받아 열고, `Full Brightness.app`을 `Applications`로 드래그합니다.
...
DMG와 앱은 직접 배포를 위해 Developer ID로 서명되어 있고 Apple notarization을 통과한 빌드입니다. ZIP asset도 자동화용으로 함께 발행하지만, 기본 설치 경로는 DMG입니다.
...
기본 빌드는 **Direct 배포용**입니다. Apple 내장 디스플레이와 Apple 네이티브 밝기 경로를 제어하기 위해 private `DisplayServices` 엔트리 포인트를 런타임 심볼 로딩으로 사용합니다.
...
기본적으로 빌드 스크립트는 private `DisplayServices` backend가 포함된 `Debug` 구성을 사용합니다.
...
Apple 번들 버전 필드는 표준 숫자 형식을 사용합니다:
...
이 스크립트는 public 직접 배포를 위해 `Developer ID Application` 인증서를 요구합니다. `Release`를 archive하고, `packaging/ExportOptions-DeveloperID.plist`로 export한 뒤, 앱을 `Full Brightness.app`으로 staging하고 DMG와 ZIP을 만들며 SHA-256과 서명 검증 결과를 출력합니다. `NOTARYTOOL_PROFILE`을 지정하면 `xcrun notarytool`로 앱과 DMG를 제출하고 둘 다 staple한 뒤 최종 artifact를 만듭니다.
...
3. 자동화나 문제 해결용 ZIP asset
4. 자동 업데이트가 필요해질 때 Sparkle 추가

Apple의 Developer ID 흐름은 직접 배포하는 앱을 Developer ID로 서명하고 notarize하는 것을 전제로 합니다. Homebrew Cask는 version, SHA-256, URL, 메타데이터, `app` artifact를 담은 cask 파일을 사용합니다.
